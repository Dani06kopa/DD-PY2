...
class TV_series:
    def __int__(self, timing: int, episodic: int):
        ...
        :param timing: хронометраж в минутах
        :param episodic: количество эпизодов
        Примеры:
        >>> tv_series = TV_series(26, 12)  # инициализация экземпляра класса
        """
    ...
    def TV_series_genre(self) -> str:
        ...
        Примеры:
        >>> tv_series = TV_series(26, 12)
        >>> tv_series.TV_series_genre("Драмма")
        """


    def is_fatherland_casket(self) -> bool:
        """
        Функция, проверяющая, является ли сериал Российским
        :return: "Да/Нет"
        Примеры:
        >>> tv_series = TV_series(26, 12)
        >>> tv_series.is_fatherland_casket('Нет')
        """
